...
const NAV_LABELS: Record<string, string> = {
    ...
    '/admin/live-orders':  'الطلبات المباشرة',
    '/admin/categories':   'الأقسام',
    '/admin/customers':    'العملاء',
    '/admin/best-sellers': 'الأكثر مبيعاً',
}
...
function InnerLayout({ children, currentUser, notifications }: Props) {
    ...
    return (
        <div dir="rtl" className="flex h-screen overflow-hidden" style={{ background: '#0a0a0b' }}>
                ...
                >
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-10 h-10 rounded-xl flex items-center justify-center text-white/65 hover:text-white hover:bg-white/10 transition-all duration-200 active:scale-95 shrink-0"
                        >
                            {isMobile ? <Menu className="w-5 h-5" /> : <PanelRightOpen className="w-5 h-5" />}
                        </button>

                        <div>
                            <h1 className="text-[15px] md:text-[18px] font-bold text-white leading-tight">{pageTitle}</h1>
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        {hasNewOrders && (
                            ...
                            >
                                ...
                                <span className="text-[12px] text-blue-400 font-bold">{notifications.newOrders} جديد</span>
                            </div>
                        )}
                        ...
                        <button
                            ...
                            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
                            className="relative w-10 h-10 rounded-xl flex items-center justify-center text-white/65 hover:text-white hover:bg-white/10 transition-all duration-200 active:scale-95"
                        >
                            <Bell className="w-5 h-5" />
                            {totalNotifs > 0 && (
                                <span
                                    style={{ background: '#ef4444', border: '2px solid #0a0a0b' }}
                                    className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full text-[9px] text-white font-black flex items-center justify-center"
                                >
